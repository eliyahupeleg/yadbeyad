...
DOMTokenList.prototype.replace=ClassList.prototype.replace})();(function(window,document,undefined){var cookieNotice=new function(){this.cookiesAccepted=null;this.noticeContainer=null;this.setStatus=function(cookieValue){var _this=this;if(cnArgs.onScroll==='1')
...
document.cookie=cnArgs.cookieName+'='+cookieValue+';expires='+laterDate.toUTCString()+';'+(!!cnArgs.cookieDomain?'domain='+cnArgs.cookieDomain+';':'')+(!!cnArgs.cookiePath?'path='+cnArgs.cookiePath+';':'')+(cnArgs.secure==='1'?'secure;':'');this.cookiesAccepted=cookieValue==='true';var event=new CustomEvent('setCookieNotice',{detail:{value:cookieValue,time:date,expires:laterDate,data:cnArgs}});document.dispatchEvent(event);this.setBodyClass(['cookies-set',cookieValue==='true'?'cookies-accepted':'cookies-refused']);this.hideCookieNotice();if(cnArgs.revokeCookiesOpt==='automatic'){this.noticeContainer.addEventListener('animationend',function handler(){_this.noticeContainer.removeEventListener('animationend',handler);_this.showRevokeNotice()});this.noticeContainer.addEventListener('webkitAnimationEnd',function handler(){_this.noticeContainer.removeEventListener('webkitAnimationEnd',handler);_this.showRevokeNotice()})}
...
return elem}
return null};this.init=function(){var _this=this;this.cookiesAccepted=this.getStatus(!0);this.noticeContainer=document.getElementById('cookie-notice');var cookieButtons=document.getElementsByClassName('cn-set-cookie'),revokeButtons=document.getElementsByClassName('cn-revoke-cookie'),closeIcon=document.getElementById('cn-close-notice');this.noticeContainer.classList.add('cn-effect-'+cnArgs.hideEffect);_this.adjustOffset();window.addEventListener('resize',function(event){_this.adjustOffset()});if(cnArgs.coronabarActive==='1'){document.addEventListener('display.coronabar',function(event){_this.adjustOffset()});document.addEventListener('hide.coronabar',function(event){_this.adjustOffset()});document.addEventListener('saveData.coronabar',function(event){var casesData=event.detail;if(casesData!==null){var request=new XMLHttpRequest();request.open('POST',cnArgs.ajaxUrl,!0);request.setRequestHeader('Content-Type','application/x-www-form-urlencoded;');request.onload=function(){if(this.status>=200&&this.status<400){}else{}};request.onerror=function(){};request.send('action=cn_save_cases&nonce='+cnArgs.nonce+'&data='+JSON.stringify(casesData))}})}
if(this.cookiesAccepted===null){if(cnArgs.onScroll==='1')
window.addEventListener('scroll',function(e){_this.handleScroll()});if(cnArgs.onClick==='1')
...
_this.setStatus('accept')},!0);this.setBodyClass(['cookies-not-set']);this.showCookieNotice()}else{this.setBodyClass(['cookies-set',this.cookiesAccepted===!0?'cookies-accepted':'cookies-refused']);if(cnArgs.revokeCookies==='1'&&cnArgs.revokeCookiesOpt==='automatic')
...
if(closeIcon!=='null'){closeIcon.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();_this.setStatus(this.dataset.cookieSet)})}
